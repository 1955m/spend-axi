...
describe("mapGatewayError", () => {
  ...
  it("maps 403 to FORBIDDEN", () => {
    const e = mapGatewayError(403, '{"error":{"message":"forbidden"}}', "x");
    expect(e.code).toBe("FORBIDDEN");
  });
  it("maps 404 to NOT_FOUND", () => {
    const e = mapGatewayError(404, '{"detail":"Not Found"}', "x");
    expect(e.code).toBe("NOT_FOUND");
  });
  it("maps 429 to RATE_LIMITED", () => {
    const e = mapGatewayError(429, '{"error":{"message":"slow down"}}', "x");
    expect(e.code).toBe("RATE_LIMITED");
  });
  it("maps 5xx to UNKNOWN", () => {
    const e = mapGatewayError(500, '{"error":{"message":"boom"}}', "x");
  ...
  it("maps an unknown status to UNKNOWN", () => {
    const e = mapGatewayError(418, '{"error":{"message":"teapot"}}', "x");
    expect(e.code).toBe("UNKNOWN");
  });
  it("handles non-JSON body", () => {
    const e = mapGatewayError(500, "Internal Server Error", "x");
    expect(e.code).toBe("UNKNOWN");
  });
});
