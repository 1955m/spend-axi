...
/**
 * Install agent session integrations. Mirrors cloudflare-axi's / tg-axi's setup
 * command. `setup hooks` installs SessionStart hooks (Claude Code / Codex /
 * OpenCode) via the SDK's installSessionStartHooks, which writes the current
 * executable path portably and is idempotent. Explicit opt-in only — never run
 * from an ordinary command (AXI P7).
 */
export async function setupCommand(args: string[], _ctx: SpendContext): Promise<string> {
  const action = args[0];
  if (action === "hooks") {
    ...
    return renderOutput([
      ...
      renderHelp(["Restart your agent session to receive spend-axi ambient context"]),
    ]);
  }
  return renderError(`Unknown setup action: ${action ?? "(none)"}`, "VALIDATION_ERROR", [
    "Run `spend-axi setup hooks` to install SessionStart hooks",
